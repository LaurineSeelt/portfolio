...
```
<br>
Then, I stored the three parameters as objects so I can use them in the title of my figures.
<br>
```{r making an object of the country, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
country <- params$country
year <- params$year
# params$month <- (1:12)
levels(params$month) <- c("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December")
# months <- params$month
```
<br>
...
covid19_data %>% group_by(day, month) %>% filter(countriesAndTerritories == params$country & year == params$year & month == params$month) %>%
