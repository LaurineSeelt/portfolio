...
gapminder <- gapminder %>% rename(year_g = "year")
```
<br>
...
flu_tidy %>% arrange(desc(flu_cases))
...
dengue_tidy %>% arrange(desc(dengue_cases))
...
gapminder %>% select(country, year_g, life_expectancy) %>% arrange(desc(life_expectancy))
gapminder %>% select( country, year_g, infant_mortality) %>% arrange(desc(infant_mortality))
```
<br>
