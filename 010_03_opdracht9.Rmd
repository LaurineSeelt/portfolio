...
```{r loading the packages3, message=FALSE, warning=FALSE, include=FALSE}
library(tidyverse)
library(here)
library(ggplot2)
```
...
<br>
```{r reading in the covid-19 data3, message=FALSE, warning=FALSE}
...
<br>
```{r making an object of the country3, message=FALSE, warning=FALSE}
covid19_data <- covid19_data %>% mutate(month = month.name[as.numeric(month)])
#country <- params$country
#year <- params$year
#months <- params$month
parameters <- list()
parameters$country <- "Austria"
parameters$year <- 2022
parameters$month <- "February"
...
<br>
```{r visualising the covid-19 cases3, message=FALSE, warning=FALSE}
...
```{r visualising the covid-19 related deaths3, message=FALSE, warning=FALSE}
covid19_data %>% group_by(day, month, year) %>% filter(countriesAndTerritories == parameters$country & year == parameters$year & month == parameters$month) %>%
...
<div>
<a href='ireland_params.html#ireland_params'>For a parameterized HTML on the COVID-19 cases and deaths in Ireland, August 2020, click here<a/>
